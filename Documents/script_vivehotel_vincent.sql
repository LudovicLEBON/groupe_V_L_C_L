...
alter table hotel  add constraint cs1 foreign key (hot_standing) references standing(sta_id) on delete cascade;
alter table chambre add constraint cs2 foreign key (cha_categorie) references categorie(cat_id) on delete cascade;
alter table chambre add constraint cs3 foreign key (cha_hotel) references hotel(hot_id) on delete cascade;
alter table individu add constraint cs4 foreign key (ind_profil) references profil(pro_id) on delete cascade;
alter table individu add constraint cs5 foreign key (ind_hotel) references hotel(hot_id) on delete cascade;
alter table client add constraint cs6 foreign key (cli_profil) references profil(pro_id) on delete cascade;
alter table reservation add constraint cs7 foreign key (res_client) references client(cli_id) on delete cascade;
alter table reservation add constraint cs8 foreign key (res_hotel) references hotel(hot_id) on delete cascade;
alter table reservation add constraint cs9 foreign key (res_chambre) references chambre(cha_id) on delete cascade;
alter table donnerAcces add constraint cs10 foreign key (don_reservation) references reservation(res_id) on delete cascade;
alter table donnerAcces add constraint cs11 foreign key (don_service) references services(ser_id) on delete cascade;
alter table prestation add constraint cs12 foreign key (pre_service) references services(ser_id) on delete cascade;
alter table prestation add constraint cs13 foreign key (pre_hotel) references hotel(hot_id) on delete cascade;
alter table tarifer add constraint cs14 foreign key (tar_standing) references standing(sta_id) on delete cascade;
alter table tarifer add constraint cs15 foreign key (tar_categorie) references categorie(cat_id) on delete cascade;

-- génération données table profil 
insert into profil values (null,"visiteur"),(null,"personnel"),(null,"SRC"),(null,"gerant"); 
